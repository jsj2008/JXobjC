...
BOOL istypeword (id sym) { return [trlunit istypeword:sym]; }

BOOL isbuiltinfun (id sym) { return [trlunit isbuiltinfun:sym]; }
...
- (BOOL)isid
{
    ...
    if (self == s_id)
        return YES;
    if (strcmp ([self str], "id") == 0)
        return YES;
    d = [trlunit lookuptype:self];
    return (d) ? [d isid] : NO;
}

- (BOOL)isvolatile { return strcmp ([self str], "volatile") == 0; }
...
- (BOOL)isgnuattrib { return NO; }

- (BOOL)isenumtor { return NO; }

- (BOOL)istypedef { return strcmp ([self str], "typedef") == 0; }

- (BOOL)isstatic { return strcmp ([self str], "static") == 0; }

- (BOOL)isextern
{
    /* this works for extern "C" as well */
    return (strcmp ([self str], "extern") == 0) ||
           (strstr ([self str], "extern") != NULL);
}
...
- (BOOL)isinline
{
    ...
    return (strcmp (token, "inline") == 0) ||
           (strcmp (token, "__inline") == 0) ||
           (strcmp (token, "__inline__") == 0);
}
...
- notrlunit { return [self error:"evaluation outside translation unit"]; }
...
- zero
{
    ...
    if ([self isid])
        return nil;
    if ([self isEqual:s_int])
        return [[Scalar new] u_int:0];
    if ([self isEqual:s_char])
        return [[Scalar new] u_char:0];
    if ([self isEqual:s_str])
...
- peekAt:(char *)ptr
{
    ...
    s = [Scalar new];
    if ([self isEqual:s_int])
        return [s u_int:*((int *)ptr)];
    if ([self isEqual:s_char])
        return [s u_char:*((char *)ptr)];
    if ([self isEqual:s_str])
        return [s u_str:*((char **)ptr)];
    if (!trlunit)
        [self notrlunit];
    d = [trlunit lookuptype:self];
    if (d)
