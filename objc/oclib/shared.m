...
- switchOn:(BOOL)dosw:e
{
    ...
    if (datadefs)
        [datadefs elementsPerform:@selector (go)];
    i = 0;
    n = [stmts size];
    if (dosw)
        for (; i < n; i++)
        {
            if ([[stmts at:i] iscaseOf:e])
                break;
        }
    for (; i < n; i++)
    {
        [[stmts at:i] go];
        if ((quit = [frame quitframe]))
            break;
        if ((docont = [frame contframe]))
            break;
        if ((dobrk = [frame breakframe]))
            break;
    }
    [Stackframe pop];
    if (quit)
...
- switchOn:e { return [self switchOn:YES:e]; }

- go { return [self switchOn:NO:nil]; }
