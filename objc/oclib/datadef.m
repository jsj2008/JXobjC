...
- (BOOL)istypedef { return istypedef; }
    ...
    for (i = 0, n = [decllist size]; i < n; i++)
    {
        ...
        if (var)
        {
            ...
            if (istypedef)
            {
                if (curdef)
                {
                    warn ("typedef not allowed inside function or method "
                          "definition");
                }
                else
                {
            ...
            else
            {
                ...
                if (gdef)
                {
                    [gdef type:t];
                    if ([d isinit])
                        [gdef initializer:[d initializer]];
                }
                ...
                if (o_refcnt && ([t isid] || [t isrefcounted]))
                {
                    if ([d isinit])
                    {
                        if (![[d initializer] isconstexpr])
                            [d incref];
                        [d cast:t];
                    }
                    else
                    {
                        /* for extern and static, or K&R parms, don't add =
                         * (id)0 */
                        if (!isextern && !isstatic && !iskandr)
                        {
                            d = [[[InitDecl new] decl:d] initnilWithType:t];
                            [decllist at:i
                                     put:d]; /* make an init : id foo = nil */
                        }
                    }
                }
            }
    ...
    while (n--)
    {
        ...
        if ([d isinit])
        {
            if ([d islistinit])
            {
                ...
                {
                    char * m = "list initializers not yet allowed in function "
                               "with Blocks";

                    fatalat (v, m);
                } /* if it is const and not for a heapvar, then doesnt need
                     remove */
            }
            else
            {
