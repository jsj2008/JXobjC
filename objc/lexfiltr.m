 ...
 *                tokens
 *
 * But in this case I've used the LEX macro YY_DECL to rename the yylex()
 * function
 ...
 * to handle special conditions that can't easily be handled within LEX and
 * YACC.
 *
 *          LEX -----------> lexfilter ----------> YACC
 *                tokens                 tokens
 *
 *
 * The problem to be handled is that of typedef names.  Whether a typedef'ed
 * name
 ...
 *
 *    'FOO' is not the target, because it is not followed by a semicolon or
 * comma.
 *
 ...
 */
static BOOL same_nesting (const struct nesting * n0, const struct nesting * n1)
{
    return ((n0->braces == n1->braces) && (n0->brackets == n1->brackets) &&
            (n0->parens == n1->parens));
}
    ...
    else
    {
        ...
        case identifier:
            ...
            else if (istypeword (token.yylval))
            {
                 ...
                 */
                if (typedef_count >
                    2) /* not the very first token after 'typedef' */
                {
                    /*
                    ...
                    {
                        token.filtered =
                            identifier; /* demote back to identifier */
                    }
                     ...
                     */
                             ...
                             (lparen_count == 1) && (nexttoken.original == ')'))
                    {
                        token.filtered =
                            identifier; /* demote back to identifier */
                    }
