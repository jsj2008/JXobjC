 ...
 * $Id: rectangl.m,v 1.1.1.1 2000/06/07 21:09:25 stes Exp $
 */
...
+ new { return [self origin:[Point new] corner:[Point new]]; }

+ origin:p corner:q { return [[super new] origin:p corner:q]; }
...
+ origin:p extent:q { return [[super new] origin:p extent:q]; }
...
- copy { return [[super copy] origin:[origin copy] corner:[corner copy]]; }
...
- origin:(int)x1:(int)y1 { return [self origin:[Point x:x1 y:y1]]; }

- corner:(int)x1:(int)y1 { return [self corner:[Point x:x1 y:y1]]; }
...
- (int)width { return [corner x] - [origin x]; }

- (int)height { return [corner y] - [origin y]; }

- origin { return origin; }

- topLeft { return origin; }

- topRight { return [Point x:[corner x] y:[origin y]]; }

- topCenter { return [Point x:[corner x] + [self width] / 2 y:[origin y]]; }

- corner { return corner; }

- bottomLeft { return [Point x:[origin x] y:[corner y]]; }

- bottomCenter { return [Point x:[corner x] + [self width] / 2 y:[corner y]]; }

- bottomRight { return corner; }
...
- extent { return [Point x:[self width] y:[self height]]; }
...
- align:p1 with:p2
{
    int x = [p2 x] - [p1 x];
    int y = [p2 y] - [p1 y];
    return [isa origin:[origin x] +
                     x:[origin y] + y corner:[corner x] + x:[corner y] + y];
}
