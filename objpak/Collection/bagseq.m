 ...
 * $Id: bagseq.m,v 1.1.1.1 2000/06/07 21:09:25 stes Exp $
 */
...
static int objbagseq_size (objbagseq_t self) { return size (self->set); }

static int cntat (objbag_t self, int i)
{
    assert (0 <= i && i < self->capacity);
    return (self->cnts)[i];
}

static id at (objbag_t self, int i)
{
    assert (0 <= i && i < self->capacity);
    return (self->ptr)[i];
}

- (unsigned)size { return (unsigned)objbagseq_size ([self objbagseqvalue]); }
...
static int match (objbag_t self, int i)
{
    return ptrmatch (self->ptr, i, self->capacity);
}

static id next (objbagseq_t self)
{
    if (self->cnt > 0)
    {
        self->cnt--;
...
- next { return next ([self objbagseqvalue]); }

static id peek (objbagseq_t self)
{
    if (self->cnt)
    {
        return at (self->set, self->offset);
...
- peek { return peek ([self objbagseqvalue]); }
...
static int prev (objbag_t self, int i) { return ptrprev (self->ptr, i); }
...
static int last (objbag_t self) { return prev (self, self->capacity); }
