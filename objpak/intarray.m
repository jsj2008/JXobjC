...
static void fill (int * p, int v, int n)
{
    while (n--)
    {
        *p++ = v;
...
static void intncpy (int * p, int * q, int n)
{
    while (n--)
    {
        *p++ = *q++;
...
static void copy (intary_t dst, intary_t src)
{
    int n = src->capacity;
...
static void clear (intary_t self)
{
    OC_Free (self->ptr);
    self->ptr = NULL;
}
...
- (intary_t)intaryvalue { return (&value); }

- (unsigned)size { return (&value)->capacity; }

static int intat (intary_t self, int i)
{
...
- (int)intAt:(unsigned)anOffset { return intat ((&value), anOffset); }

static int intputint (int * self, int c)
{
    int r = *self;
    *self = c;
    return r;
...
static int putintat (intary_t self, int i, int c)
{
    if (0 <= i && i < self->capacity)
    {
...
static void resize (intary_t self, unsigned c)
{
    int min, n;
...
static void removezeroes (int * to, int * limit)
{
    int * from;
...
static void packcontents (intary_t self)
{
...
static void intfileout (id aFiler, int * p, int n)
{
    while (n--)
    {
...
static void intfilein (id aFiler, int * p, int n)
{
    while (n--)
    {
...
static void fileout (id aFiler, intary_t self)
{
    [aFiler fileOut:&self->capacity type:'i'];
    intfileout (aFiler, self->ptr, self->capacity);
}

static void filein (id aFiler, intary_t self)
{
